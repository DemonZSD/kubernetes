...
// params
//   kubeDeps: kubelet 在运行时所必须的依赖 （依赖注入）
//   nodeName: 每个node回会运行一个kubelet服务，而EventRecorder是kubelet的一个子组件
func makeEventRecorder(kubeDeps *kubelet.Dependencies, nodeName types.NodeName) {
	if kubeDeps.Recorder != nil {  // 如果依赖组间中 EventRecorder 不为空，则直接返回
		return
	}
	// new一个 eventBroadcasterImpl, 用于广播 Events, 记录到两个地方，1.记录到日志里 ；2.通过api-server记录到 ETCD
	eventBroadcaster := record.NewBroadcaster()
	// new 一个 EventRecorder, 用于记录日志 ——> 提供了 Event ，Eventf 等方法供事件记录
	// v1.EventSource 为 事件的来源，比如kubelet中的某个节点
	kubeDeps.Recorder = eventBroadcaster.NewRecorder(legacyscheme.Scheme, v1.EventSource{Component: componentKubelet, Host: string(nodeName)})
	// 对Events的消费1： 从 EventBroadcaster 接收来的Events, 将其发送到 log的Infof 方法，
	// Note: 其返回值可以忽略或者用于停止消费日志
