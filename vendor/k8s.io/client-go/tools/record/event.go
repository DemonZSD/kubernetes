...
// and EventAggregator（聚合器） in EventCorrelator
type CorrelatorOptions struct {
	// The lru（最近最少使用） cache size used for both EventSourceObjectSpamFilter and the EventAggregator
	...
	BurstSize int
	// The fill rate of the token bucket in queries per second（每秒查询） in EventSourceObjectSpamFilter
	// If not specified (zero value), the default specified in events_cache.go will be picked
...
func NewBroadcaster() EventBroadcaster {
	return &eventBroadcasterImpl{
		// 实例化 Broadcaster
		// maxQueuedEvents 每个watcher的queue最大的 event 数量
		Broadcaster:   watch.NewBroadcaster(maxQueuedEvents, watch.DropIfChannelFull),
		sleepDuration: defaultSleepDuration, // 默认睡眠时间
	}
}
...
func recordToSink(sink EventSink, event *v1.Event, eventCorrelator *EventCorrelator, sleepDuration time.Duration) {
	...
	eventCopy := *event  // eventCopy 为值copy
	event = &eventCopy  // event 指向新值的地址
	//
	result, err := eventCorrelator.EventCorrelate(event)
	if err != nil {
		utilruntime.HandleError(err)
	}
	// 如果 Skip为 true, 则对 event 不做进一步处理
	if result.Skip {
		return
	}
	tries := 0 // 重试次数， 在api-server 无法连通的情况下，会尝试多次
	for {
		// 如果 记录事件 false， 则下次循环继续尝试，直到最大次数
		// result.Event.Count > 1， 用于判断event 是否已存在，如果存在则更新，不存在则新建一个event
		...
		if tries == 1 {
			// 通过睡眠随机时间，避免 所有的 clients 在api-server恢复通信后，第一时间一拥而上的进行请求交互，会对api-server产生压力
...
// recordEvent attempts to(意图) write event to a sink. It returns true if the event
// was successfully recorded or discarded（丢弃）, false if it should be retried.
// If updateExistingEvent is false, it creates a new event, otherwise it updates
// existing event.
func recordEvent(sink EventSink, event *v1.Event, patch []byte, updateExistingEvent bool, eventCorrelator *EventCorrelator) bool {
	var newEvent *v1.Event
	var err error
	if updateExistingEvent { // it creates a new event, otherwise it updates existing event.
...
func (e *eventBroadcasterImpl) StartLogging(logf func(format string, args ...interface{})) watch.Interface {
	// 调用 eventBroadcasterImpl的StartEventWatcher方法， 进行log对日志的记录
	return e.StartEventWatcher(
		func(e *v1.Event) {  //func(e *v1.Event) 通过方法参数传递，进行event的日志记录操作
...
func (e *eventBroadcasterImpl) StartEventWatcher(eventHandler func(*v1.Event)) watch.Interface {
	watcher := e.Watch()  //获取 eventBroadcasterImpl中的 watcher
	go func() {
		defer utilruntime.HandleCrash()
		for watchEvent := range watcher.ResultChan() {  // 遍历 watcher中的chan队列，获取event
			...
			eventHandler(event)  // 处理日志 eventHandler 方法用于处理日志的
		}
	}()
	return watcher
}
